...
export class DiscussCardComponent implements OnInit {
 ...
 /* to handle like and dislike */ 
  ...
  editReplyPost(post:any,masterData:any){
    ...
    this.contentPost.content = get(post, 'content').replace(/<[^>]*>/g, '');
  ...
  editReplyHandler(event) {
    if (get(event, 'action') === 'cancel') {
      ...
      this.updatePost(get(event, 'content'), get(this.contentPost.post, 'pid'));
    }
  }
  updatePost(updatedPostContent: any, pid: number) {
    ...
    const req = {
      ...
      title: '',
      tags: [],
      uid: get(this.contenPostData, 'loggedInUser.uid')
    };
